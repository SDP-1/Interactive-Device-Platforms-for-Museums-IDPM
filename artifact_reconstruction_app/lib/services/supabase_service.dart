...
class SupabaseService {
  ...
  Future<void> updateArtifactStatus({
    ...
    await _client.from('artifacts').update(updates).eq('id', id);
  }
  ...
  Future<String> uploadImage(String artifactId, File imageFile) async {
    ...
    await _client.storage.from('artifacts').upload(filePath, imageFile);
    return _client.storage.from('artifacts').getPublicUrl(filePath);
  }
  ...
  Future<String> uploadModel(String artifactId, File modelFile) async {
    ...
    await _client.storage.from('artifacts').upload(filePath, modelFile);
    return _client.storage.from('artifacts').getPublicUrl(filePath);
  }
}
