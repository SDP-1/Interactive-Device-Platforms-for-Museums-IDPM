...
class HistoricalDataLoader:
    ...
    def load_data(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        dtype_dict = {
            ...
            'event_name': 'object',
            'exhibit_name': 'object',
            # IMPORTANT: keep date as raw string so BCE/century/range values are preserved (not coerced to NaT)
            'date': 'object',
        }
        self.nodes_df = pd.read_csv(self.nodes_file, dtype=dtype_dict)
        self.edges_df = pd.read_csv(self.edges_file, dtype=dtype_dict)
        # Do NOT coerce nodes_df['date'] to datetime here.
        # Temporal comparisons are handled via date_utils.year_for_ordering where needed.
    ...
    def calculate_temporal_gap_flexible(self, source_date_raw, target_date_raw) -> float:
        """
        Calculate time gap in days between two events, supporting:
        - pandas timestamps
        - ISO date strings
        - BCE/CE/century/range strings (via date_utils.year_for_ordering)
        """
        try:
            # If both parse as datetimes, prefer that
            sdt = pd.to_datetime(source_date_raw, errors="coerce")
            tdt = pd.to_datetime(target_date_raw, errors="coerce")
            if pd.notna(sdt) and pd.notna(tdt):
                return float(abs((tdt - sdt).days))
        except Exception:
            pass

        sy = year_for_ordering(str(source_date_raw or ""))
        ty = year_for_ordering(str(target_date_raw or ""))
        if sy is None or ty is None:
            return 0.0
        return float(abs(ty - sy) * 365.25)
    ...
    def build_graph(self) -> Data:
        ...
        for _, edge_row in self.edges_df.iterrows():
            ...
            temporal_gap = 0.0
            if len(source_node_data) > 0 and len(target_node_data) > 0:
                temporal_gap = self.calculate_temporal_gap_flexible(source_node_data.iloc[0]['date'], target_node_data.iloc[0]['date'])
