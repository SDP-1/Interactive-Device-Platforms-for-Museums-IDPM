...
class GraphConstructor:
    ...
    def construct_subgraph(
        ...
        evidence: Dict,
        local_event_data: Optional[Dict] = None,
    ) -> Dict:
        """
        ...
        local_node = self._get_local_node(local_event_id, local_event_data=local_event_data)
        if local_node:
            graph['nodes'].append({
                'id': local_event_id,
                'type': 'local',
        ...
        if local_node:
            local_text = " ".join([
                str(local_node.get('event_name', '') or ''),
                str(local_node.get('description', '') or ''),
                str(local_node.get('location', '') or ''),
                str(local_node.get('exhibit_name', '') or ''),
            ]).lower()
        ...
        intermediate_nodes = self._extract_intermediate_nodes(candidates, evidence, local_text=local_text)
    ...
    def _get_local_node(self, local_event_id: str, local_event_data: Optional[Dict] = None) -> Optional[Dict]:
        """Get local node data."""
        ...
        # Fallback: allow ad-hoc local events not present in the base nodes CSV.
        if local_event_data and isinstance(local_event_data, dict):
            return {
                'node_id': local_event_id,
                'event_name': local_event_data.get('event_name', local_event_data.get('title', '')) or '',
                'date': str(local_event_data.get('date', local_event_data.get('start_date', '')) or ''),
                'location': local_event_data.get('location', '') or '',
                'description': local_event_data.get('description', '') or '',
                'exhibit_name': local_event_data.get('exhibit_name', '') or '',
                'source_url': local_event_data.get('source_url', '') or '',
            }
        return None
    ...
    def _extract_intermediate_nodes(
        ...
        local_text = (local_text or "").lower()

        # Extract commodities (ONLY if relevant to the local event text).
        # This prevents nonsense paths like "Theravada affected Tea" just because a Wikipedia
        # snippet mentions tea somewhere.
        commodities = set()
        if 'tea' in local_text:
            commodities.add('tea')
        if 'coffee' in local_text:
            commodities.add('coffee')
        if 'cotton' in local_text:
    ...
    def _create_preliminary_edges(
        ...
        local_node = next((n for n in graph['nodes'] if n['id'] == local_event_id), None)
        local_text = ""
        if local_node and isinstance(local_node.get('data'), dict):
            d = local_node['data']
            local_text = " ".join([
                str(d.get('event_name', '') or ''),
                str(d.get('description', '') or ''),
                str(d.get('location', '') or ''),
                str(d.get('exhibit_name', '') or ''),
            ]).lower()
        ...
        for inter_node in intermediate_nodes[:5]:  # Top 5 intermediates
            ...
            inter_type = (inter_node.get('data') or {}).get('type', '')
            inter_name = str((inter_node.get('data') or {}).get('name', '') or '').lower()
            
            # Connect global events to intermediate
            for candidate in candidates[:10]:
                global_id = candidate['global_event']['node_id']
                if global_id in graph['node_to_idx']:
                    # Only create the link if the global event actually mentions this intermediate.
                    ge = candidate.get('global_event', {}) or {}
                    ge_text = " ".join([
                        str(ge.get('event_name', '') or ''),
                        str(ge.get('description', '') or ''),
                        " ".join([str(k) for k in (ge.get('keywords', []) or [])]),
                    ]).lower()

                    if inter_name:
                        if inter_type == 'commodity':
                            if inter_name not in ge_text:
                                continue
                        elif inter_type == 'entity':
                            if inter_name not in ge_text:
                                continue
            ...
            if local_event_id in graph['node_to_idx']:
                # Only connect if the local event text mentions this intermediate.
                if inter_name and (inter_name not in local_text):
                    continue
                edge = {
                    'edge_id': f"EDGE_{edge_idx}",
                    'source': inter_id,
