...
from date_utils import parse_year_range, year_for_ordering
...
class ConstraintScorer:
    ...
    def _apply_constraints(self, prediction: Dict, graph: Dict) -> Dict:
        ...
        try:
            ...
            if global_date and local_date and global_date != 'nan' and local_date != 'nan':
                global_year = year_for_ordering(global_date)
                local_year = year_for_ordering(local_date)
                
                if global_year is not None and local_year is not None:
                    ...
                    global_name = str(global_event.get('event_name', '') or '').lower()
                    local_name = str(local_node['data'].get('event_name', '') or '').lower()
                    ...
                    if 'coffee leaf rust' in global_name and global_year == 1869:
                        ...
                        # Ancient BCE dates are often approximate and differ by source/tradition.
                        # Allow a small tolerance window to avoid false temporal failures.
                        if global_year < 0 and local_year < 0:
                            constraints['temporal_order'] = global_year <= (local_year + 100)
                        else:
                            constraints['temporal_order'] = global_year <= local_year
                else:
        ...
        global_location = str(global_event.get('location', '') or '').lower()
        local_location = str(local_node['data'].get('location', '') or '').lower()
    ...
    def _calculate_evidence_strength(
        ...
        global_event = prediction.get('metadata', {})
        event_name = str(global_event.get('event_name', '') or '').lower()
        description = str(global_event.get('description', '') or '').lower()
                    ...
                    if not global_date or not local_date or global_date.lower() == 'nan' or local_date.lower() == 'nan':
                        ...
                        try:
                            ...
                            if pd.notna(global_dt) and pd.notna(local_dt):
                                ...
                                # Fallback: parse BCE/CE/century/ranges
                                gy = year_for_ordering(global_date)
                                ly = year_for_ordering(local_date)
                                if gy is not None and ly is not None:
                                    temporal_gap = abs(ly - gy) * 365.25
                        except Exception as e:
