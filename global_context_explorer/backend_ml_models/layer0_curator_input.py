...
class CuratorInputParser:
    ...
    def __init__(self):
        self.date_patterns = [
            ...
            r'\b(\d{1,4})\s*(BCE|CE)\b',  # 247 BCE
            r'\b(\d{1,2})(st|nd|rd|th)\s*century\b',  # 3rd century
        ]
    ...
    def parse(self, input_text: str, date: Optional[str] = None, location: Optional[str] = None) -> Dict:
        ...
        """
        normalized_input_text = self._normalize_text(input_text or "")

        query = {
            'local_event_text': normalized_input_text.strip(),
            'date_range': None,
            'location': None,
            'entities': [],
            'keywords': []
        ...
        if date:
            ...
            extracted_date = self._extract_date_from_text(normalized_input_text)
            if extracted_date:
        ...
        if location:
            ...
            extracted_location = self._extract_location_from_text(normalized_input_text)
            if extracted_location:
                query['location'] = extracted_location
        
        # Extract entities and keywords
        query['entities'] = self._extract_entities(normalized_input_text)
        query['keywords'] = self._extract_keywords(normalized_input_text)
        
        return query

    def _normalize_text(self, text: str) -> str:
        """
        Light normalization to handle common curator typos and variations.
        """
        s = str(text or "")
    ...
    def _parse_date(self, date_str: str) -> Optional[Dict]:
        ...
        try:
            # Support BCE/CE/century/ranges via shared parser
            start_year, end_year = parse_year_range(str(date_str))
            if start_year is not None:
                # Keep the original string for display; store year for ordering/scoring
                return {
                    'start': str(date_str).strip(),
                    'end': str(date_str).strip(),
                    'year': start_year if end_year is None else start_year
                }
    ...
    def _extract_date_from_text(self, text: str) -> Optional[Dict]:
        ...
        # First: try robust range parser (handles BCE/centuries/ranges)
        try:
            start_year, _ = parse_year_range(text)
            if start_year is not None:
                return {
                    'start': text,
                    'end': text,
