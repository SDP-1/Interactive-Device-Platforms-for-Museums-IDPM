            ...
            if source_node and target_node:
                ...
                if edge and edge.get('type') == 'causal_candidate':
                    explanation_parts.append(
                        f"{source_name} influenced {target_name}"
                    )
                elif edge and edge.get('type') == 'mentions' and target_node['type'] == 'intermediate':
                    explanation_parts.append(
                        f"{source_name} mentions {target_name} ({inter_type})"
                    )
                elif edge and edge.get('type') == 'related_to' and target_node['type'] != 'intermediate':
                    explanation_parts.append(
                        f"{source_name} relates to {target_name}"
                    )
                elif target_node['type'] == 'intermediate':
                    # Fallback wording if edge type is missing
                    explanation_parts.append(
                        f"{source_name} relates to {target_name} ({inter_type})"
                    )
                else:
                    explanation_parts.append(
