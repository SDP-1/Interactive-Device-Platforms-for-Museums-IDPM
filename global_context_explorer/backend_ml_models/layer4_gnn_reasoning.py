...
class GNNReasoner:
    ...
    def __init__(self, nodes_file: str, edges_file: str):
        ...
        self.model_path = Path("models") / "causal_gnn.pt"
    ...
    def _try_load_trained_weights(self) -> None:
        """Load trained model weights if available (created by train_gnn.py)."""
        try:
            ...
            if isinstance(payload, dict) and "gnn_state_dict" in payload:
                self.model.load_state_dict(payload["gnn_state_dict"])
                print(f"[Layer 4] Loaded trained GNN weights from: {self.model_path}")
        except Exception as e:
            print(f"[Layer 4] Warning: failed to load trained model weights: {e}")
    ...
    def predict_links(
        ...
        # We can use the GNN **per-node** if that node exists in the base CSV graph.
        # If some candidates are missing from the base graph, we still score the ones that exist
        # and fallback for the rest (instead of falling back for everything).
        can_use_gnn_for_some = (
            self.path_finder is not None and
            self.graph_data is not None and
            hasattr(self.graph_data, 'node_to_idx') and
            local_event_id in self.graph_data.node_to_idx
        )
        ...
        for global_node in global_nodes:
            ...
            if can_use_gnn_for_some and global_id in self.graph_data.node_to_idx:
                try:
    ...
    def _predict_mechanism(
        ...
        if 'industrial revolution' in event_name or 'industrial revolution' in event_text:
        ...
        elif any(k in event_text for k in ['buddhism', 'buddhist', 'ashoka', 'mahinda', 'theravada', 'sangha', 'missionary']):
            mechanisms['religious_diffusion'] = 0.85
            mechanisms['policy'] = 0.15
        else:
            ...
            # Religious/cultural diffusion keywords
            religion_keywords = ['buddhism', 'buddhist', 'ashoka', 'mahinda', 'theravada', 'sangha', 'missionary', 'doctrine']
            if any(kw in event_name or kw in description for kw in religion_keywords):
                mechanisms['religious_diffusion'] = 0.75
                mechanisms['policy'] = max(mechanisms['policy'], 0.1)
    ...
    def _predict_mechanism_from_edge(self, edge: Dict, graph: Dict = None, source_node: Dict = None) -> Dict[str, float]:
        ...
        if 'industrial revolution' in event_name or 'industrial revolution' in event_text:
            ...
            # Religious/cultural diffusion keywords
            religion_keywords = ['buddhism', 'buddhist', 'ashoka', 'mahinda', 'theravada', 'sangha', 'missionary', 'doctrine']
            if any(kw in event_name or kw in event_description for kw in religion_keywords):
                mechanisms['religious_diffusion'] = 0.75
                mechanisms['policy'] = max(mechanisms['policy'], 0.1)
